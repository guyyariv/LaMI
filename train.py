...
frozen base LLM and a frozen CLIP vision encoder. Images are either loaded
from disk or generated on the fly with SDXL-Turbo for text-only batches
(WikiText).
"""
...
SUPPORTED_FAMILIES = ("llama", "gemma", "gpt2", "bert")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset_name", type=str, required=True,
                        help="Substring-matched list of {wiki, laion_220, cropped_vg}.")
    parser.add_argument("--output_dir", type=str, required=True)

    parser.add_argument("--max_seq_length", type=int, default=1024)
    parser.add_argument("--max_train_samples", type=int, default=None)
    parser.add_argument("--max_elements", type=int, default=None,
                        help="Per-dataset cap before concatenation.")
    ...
    parser.add_argument("--laion_image_root", type=str, default=None,
                        help="Root dir for LAION-220 image files (required for --dataset_name laion_220).")
...
def _truncate_to_active_tokens(input_ids, attention_mask, tokenizer, family):
...
def _mask_for_bert(input_ids, mlm_probability: float = 0.20):
...
def make_collate_fn(tokenizer, family: str):
    def collate(examples):
        ...
        if family == "bert":
            ...
            input_ids, attention_mask = _truncate_to_active_tokens(
                input_ids, attention_mask, tokenizer, family
            )
            labels = input_ids
...
def _detect_family(model_name: str) -> str:
    ...
    for key in SUPPORTED_FAMILIES:
        if key in name:
            return key
    raise ValueError(
        f"Unrecognized model family for '{model_name}'. Supported: {SUPPORTED_FAMILIES}."
    )


def _load_pretrained_weights(model, path: str) -> None:
