...
Replaces the per-task scripts. Usage:
...
def generate_visual_evidence(args, model, pipe, clip, anchor_text: str):
    ...
    if args.no_clip_rescaling:
        scores = torch.full((len(images),), 0.5, device=model.device, dtype=model.dtype)
    else:
...
def _qa_prompt(title: str, paragraph: str, context: str,
    ...
    if paragraph:
        head += f"Paragraph: {paragraph}\n\n"
    else:
        head += "\n"
    return (
        head + f"Background: {context}\n\n"
        f"Q: {former_q}\n\nA: {former_a}\n\n"
        f"Q: {question}\n\nA:"
    )
...
def _qa_iter_squad(dataset):
    ...
    for item in dataset:
        ...
        if title == former_title and former_q is not None:
            yield title, "", item["context"], former_q, former_a, question, answers
        former_title = title
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pretrained_model", default=None,
                        help="Path (without .bin) to a LaMI checkpoint.")
    ...
    parser.add_argument("--clip_temperature", type=float, default=2.0,
                        help="Scaling on cosine similarity used as per-image mixing weight.")
    parser.add_argument("--no_clip_rescaling", action="store_true",
                        help="Skip CLIP-based per-image weighting (equal mixture).")
