...
class _FusionInit:
    ...
    def _init_fusion_modules(self, config, clip_name: str = CLIP_NAME) -> None:
        ...
        # Per-instance KV cache for the fusion layer during generation.
...
class CausalLMFusionMixin(_FusionInit):
    ...
    on the inherited class (``"model"`` for Llama/Gemma/OPT, ``"transformer"``
    for GPT-2). The fusion layer is a 1-layer copy of the same architecture.
    """
    ...
    def forward(
        ...
        **base_kwargs,
        ...
        if pixel_values is not None and prompt_step:
            ...
            fused_hidden = fusion_outputs.last_hidden_state
            logits = self.lm_head(fused_hidden).float()

        elif decode_step:
            # Single-token decode after a prompted prefix: reuse cached fusion KV.
...
class MaskedLMFusionMixin(_FusionInit):
    ...
    (the MLM head). The fusion layer is a 1-layer encoder built from a copy
    of the same config.
    """
